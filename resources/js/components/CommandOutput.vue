...
export default {
  ...
  computed: {
    styledOutput() {
      ...
      // Иногда PHP выводит ESC как \u001b или \033.
      // Проверим, что библиотека видит эти символы.
      const parsed = parse(this.output);

      return parsed.spans.map(span => {
        // Если библиотека не нашла стилей, вернем просто текст
        if (!span.css) return span.text;
